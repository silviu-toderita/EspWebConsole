...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
bool EspWebConsole::handle_file_read(String path){  
    ...
    //If the file exists in the root folder instead of the /www/ folder, stream it to the client (this is for debugging non-_web_server files)
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
void EspWebConsole::websockets_event(uint8_t num, WStype_t type, uint8_t * payload, size_t length) { // When a WebSocket message is received
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
String EspWebConsole::text_input_HTML(String id, String val, String type, bool req){
...
/*  (private)multi_input_HTML: Create the html for a multiple choice input
    ...
    RETURNS complete HTML
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
String EspWebConsole::multi_input_html(String id, String val, String type, JsonArray opt){
...
/*  (private)multi_input_HTML: Create the html for all inputs in a category
    ...
    RETURNS complete HTML
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
String EspWebConsole::input_html(JsonArray settings){
    String response = "";
    //For each setting...
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
void EspWebConsole::handle_settings_get(){
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
void EspWebConsole::handle_settings_post(){
    ...
    //The current _web_server argument being processed
    uint8_t current_web_server_arg = 0;
    ...
    for(uint8_t i = 0; i < doc.size(); i++){
        ...
        //For each setting...
        for(uint8_t x = 0; x < settings.size(); x++){
            //Copy the _web_server argument to the current value and increment the _web_server argument
            settings[x]["val"] = server.arg(current_web_server_arg);
            current_web_server_arg++;
        }
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
void EspWebConsole::set_callback(void_function_pointer offline){
    _restart = offline;
}
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
void EspWebConsole::handle_file_upload(){
    ...
    //If the upload is over, send _web_server status 201 and close the file
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
bool EspWebConsole::begin(){
    //When the settings file is requested or posted, call the corresponding function
    server.on("/settings_data", HTTP_POST, [&](){
        handle_settings_post();
    });
    server.on("/settings_data", HTTP_GET, [&](){
        handle_settings_get();
    });
    server.on("/nav", HTTP_GET, [&](){
        handle_nav();
    });
    ...
    server.begin(); //Start the _web_server

    if(console_page){
        //If a websockets message comes in, call this function
        websockets_server.onEvent([&](uint8_t num, WStype_t type, uint8_t * payload, size_t length){
                websockets_event(num,type,payload,length);
            });
        websockets_server.begin(); //Start the websockets _web_server
...
/*  handle: Check for incoming requests to the _web_server and to the websockets _web_server
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
void EspWebConsole::handle(){
    server.handleClient();
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
void EspWebConsole::console_print(String output){
    if(console_page){
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
String EspWebConsole::load_setting(String setting){
    //Open the file for reading
    File file = LittleFS.open(SETTINGS_PATH, "r");
    //Set aside enough memory for a JSON document
    DynamicJsonDocument doc(file.size() * 2);
